...
import net.clozynoii.invincibleconquest.procedures.JoinCOPProcedure;
...
public record FactionJoinCOPButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static void handleData(final FactionJoinCOPButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		if (buttonID == 0) {
			...
			JoinCOPProcedure.execute(entity);
		}
		if (buttonID == 1) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(FactionJoinCOPButtonMessage.TYPE, FactionJoinCOPButtonMessage.STREAM_CODEC, FactionJoinCOPButtonMessage::handleData);
	}
}
