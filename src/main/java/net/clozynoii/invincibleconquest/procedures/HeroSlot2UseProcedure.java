...
public class HeroSlot2UseProcedure {
		...
		if (!(entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).HeroSlot2).isEmpty()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if ((entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).HeroSlot2).equals(entityiterator.getDisplayName().getString())) {
						{
