...
public class HeroSlot4UseProcedure {
		...
		if (!(entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).HeroSlot4).isEmpty()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if ((entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).HeroSlot4).equals(entityiterator.getDisplayName().getString())) {
						{
