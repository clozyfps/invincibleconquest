...
public class EnergyProjectionAbilityProcedure {
		...
		if (entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).PlayerCurrentStamina >= staminacost) {
			...
			if (Mth.nextInt(RandomSource.create(), 1, 2) == 1) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("invincible_conquest:beam23")), SoundSource.NEUTRAL, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("invincible_conquest:beam23")), SoundSource.NEUTRAL, 1, 1, false);
					}
				}
			...
			}
			{
				...
				if (!projectileLevel.isClientSide()) {
					...
					}.getArrow(projectileLevel, entity, (float) (5 + entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).PlayerStrength / 10), 2, (byte) 0);
