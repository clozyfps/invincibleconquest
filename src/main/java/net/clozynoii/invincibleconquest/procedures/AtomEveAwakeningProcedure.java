...
@EventBusSubscriber
public class AtomEveAwakeningProcedure {
	@SubscribeEvent
				...
				if (!entity.getData(InvincibleConquestModVariables.PLAYER_VARIABLES).AwakenedAtomEve) {
					if (Mth.nextInt(RandomSource.create(), 1, 500) == 2) {
						if (entity instanceof ServerPlayer _player) {
							AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("invincible_conquest:trigger_sequence"));
							if (_adv != null) {
						...
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
									"/title @s title {\"text\":\"Memory Unlocked\",\"color\":\"#ff55ff\"}");
						...
						if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(InvincibleConquestModMobEffects.ATOM_AWAKENING_EFFECT, 100, 0, false, false));
						{
